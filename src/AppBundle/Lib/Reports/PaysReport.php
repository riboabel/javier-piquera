...
class PaysReport extends Report
{
    /**
     * @var \DateTime
     */
    ...
    private function render()
    {
        ...
        foreach ($this->getQuery()->getResult() as $record) {
            ...
            $this->pdf->Cell(0, 0, sprintf('%s servicio(s) pagado(s)', $record->getCharges()->count()), 1, 1);
            ...
            foreach ($record->getCharges() as $charge) {
                ...
                $this->pdf->Cell(0, 0, sprintf('%0.2f', $charge->getDriverPayAmount()), 'BR', 1, 'R');
            }
    ...
    {
        $qb = $this->em->getRepository('AppBundle:PayAct')
        ...
        if (null !== $this->startAt) {
            $andX->add($qb->expr()->gte('p.createdAt', $qb->expr()->literal($this->startAt->format('Y-m-d'))));
        }
        if (null !== $this->endAt) {
            $andX->add($qb->expr()->lte('p.createdAt', $qb->expr()->literal($this->endAt->format('Y-m-d 23:59:59'))));
        }
